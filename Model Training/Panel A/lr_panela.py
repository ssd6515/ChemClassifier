# Model Training for Panel A: Logistic Regression with Elastic Net penalty using Dragon descriptors
import pandas as pd
import numpy as np
...
# ============================================================
# Logistic Regression ElasticNet Classification
...
print(
    ...
    "correct RepeatedStratifiedKFold"
)
...
nHM = data["nHM"].to_numpy().reshape(-1, 1)
piPC09 = data["piPC09"].to_numpy().reshape(-1, 1)
PCD = data["PCD"].to_numpy().reshape(-1, 1)
X2Av = data["X2Av"].to_numpy().reshape(-1, 1)
MLOGP = data["MLOGP"].to_numpy().reshape(-1, 1)
ON1V = data["ON1V"].to_numpy().reshape(-1, 1)
...
for repeat in range(N_REPEATS):
    ...
    for k in range(N_SPLITS):
        ...
        train_feature = scaler.fit_transform(train_feature_raw)
        valid_feature = scaler.transform(valid_feature_raw)
        test_feature = scaler.transform(test_feature_raw)
...
# ------------------------------------------------------------
# 8. Save fold metrics, predictions, and repeat-level metrics
# ------------------------------------------------------------

with open("results_lr_panela_repeat.pkl", "wb") as f:
    pickle.dump(
        {
...
with open("results_lr_panela_final_metrics.pkl", "wb") as f:
    pickle.dump(all_metrics, f)


# ------------------------------------------------------------
# 11. Select the overall best model
# ------------------------------------------------------------

best_repeat_index = np.argmin(repeat_best_val_losses)
best_overall_model = repeat_best_models[best_repeat_index]
best_overall_scaler = repeat_best_scalers[best_repeat_index]
best_repeat_hyper = repeat_best_hyperparams[best_repeat_index]

print(
    f"\nBest Overall Model from repeat {best_repeat_index}: "
    f"C = {best_repeat_hyper[0]}, l1l2_ratio = {best_repeat_hyper[1]}"
)

# Save both model and scaler.
# The scaler is needed because LR was trained on standardized features.
torch.save(
    {
        "model": best_overall_model,
        "scaler": best_overall_scaler,
        "best_hyperparameters": {
            "C": best_repeat_hyper[0],
            "l1l2_ratio": best_repeat_hyper[1],
        },
        "class_labels": class_labels,
    },
    "best_lr_model.pt",
...
print("Best overall LR ElasticNet model and scaler saved as best_lr_model.pt")
