# Model Training for Panel B: Gradient Boosting Decision Trees (GBDT) with RDKit descriptors
import pandas as pd
import numpy as np
...
# ============================================================
# GBRT Panel B Classification with Correct RepeatedStratifiedKFold
...
def build_applicability_domain_reference(
    ...
    X_train_scaled = (X_train_imputed - descriptor_mean) / descriptor_std
    ...
    knn_train.fit(X_train_scaled)

    train_distances, train_neighbor_indices = knn_train.kneighbors(X_train_scaled)
    ...
    ad_reference = {
        ...
        "training_scaled_features": X_train_scaled,
    }
...
def evaluate_applicability_domain(
    ...
    X_train_scaled = ad_reference["training_scaled_features"]
    ad_k = ad_reference["ad_k_effective"]
    ad_distance_threshold = ad_reference["ad_distance_threshold"]

    X_query_scaled = (X_query_imputed - descriptor_mean) / descriptor_std
    ...
    knn_query.fit(X_train_scaled)

    query_distances, query_neighbor_indices = knn_query.kneighbors(X_query_scaled)
...
print(
    "n_estimator = [50, 100, 200, 300, 400, 500, 600, 650, 700, 800, 900, 1000], "
    "max_depths = [2, 3, 4, 5, 6, 7, 8, 9, 10, 12, 15, 20, 25, 30], "
    "no_ecfp, gbrt_panelb_noSMOTE, mean_imputation, correct RepeatedStratifiedKFold, "
...
N_ESTIMATORS = [50, 100, 200, 300, 400, 500, 600, 650, 700, 800, 900, 1000]
MAX_DEPTHS = [2, 3, 4, 5, 6, 7, 8, 9, 10, 12, 15, 20, 25, 30]
...
# Store indices for the repeat-best models.
# These are needed to define the AD using the exact training set
# used by the selected best model.
...
# Global best tracking across all 25 outer evaluations.
...
for repeat in range(N_REPEATS):
    ...
    for k in range(N_SPLITS):
        ...
        if best_valid_loss < repeat_best_val_loss:
            ...
            repeat_best_hyper = (best_ne, best_d)
        ...
        if best_valid_loss < global_best_val_loss:
            ...
            global_best_hyperparams = (best_ne, best_d)
        ...
        all_fold_imputation_means.append(train_col_means)
    ...
    print(
        ...
        f"n_estimator = {repeat_best_hyper[0]}, max_depth = {repeat_best_hyper[1]}"
    )
...
# ------------------------------------------------------------
# 7. Save fold metrics, predictions, repeat-level metrics
# ------------------------------------------------------------
...
print(
    ...
    f"\n  n_estimator = {best_overall_hyper[0]}"
    f"\n  max_depth = {best_overall_hyper[1]}"
)

# Save model and training-column means.
# The column means are needed to impute new data before prediction.
torch.save(
    {
        ...
        "best_hyperparameters": {
            "n_estimators": best_overall_hyper[0],
            "max_depth": best_overall_hyper[1],
        },
    ...
    "best_gbdt_model_panelb_mean_imp.pt",
)

print("Best overall GBDT model and imputation means saved as best_gbdt_model_panelb_mean_imp.pt")
...
print("\n============================================================")
print("Applicability Domain Analysis for Selected Best Model")
print("============================================================")
...
ad_summary_df = pd.DataFrame(
    {
        ...
        "best_model_n_estimators": [best_overall_hyper[0]],
        "best_model_max_depth": [best_overall_hyper[1]],
        "n_ad_training_chemicals": [best_train_feature.shape[0]],
        "n_features": [best_train_feature.shape[1]],
    }
)

ad_summary_df.to_csv("gbrt_panelb_best_model_AD_summary.csv", index=False)

print("\nSaved AD summary to: gbrt_panelb_best_model_AD_summary.csv")
...
feature_range_df = pd.DataFrame(
    {
        ...
        "training_mean": ad_reference["descriptor_mean"],
        "training_std": ad_reference["descriptor_std"],
        "training_imputation_mean": best_overall_col_means,
    }
)

feature_range_df.to_csv("gbrt_panelb_best_model_training_feature_ranges.csv", index=False)

print("Saved training feature min/max ranges to: gbrt_panelb_best_model_training_feature_ranges.csv")
...
ad_training_distances_df.to_csv(
    "gbrt_panelb_best_model_training_AD_distances.csv",
    index=False,
)

print("Saved training AD distances to: gbrt_panelb_best_model_training_AD_distances.csv")


# ------------------------------------------------------------
# 12. Apply selected best model + AD to its held-out test set
# ------------------------------------------------------------

print("\n============================================================")
print("Best Model Test Set Predictions with AD Flags")
print("============================================================")
...
best_test_results_df.to_csv(
    "gbrt_panelb_best_model_test_predictions_with_AD.csv",
    index=False,
)

print("Saved best-model test predictions with AD to: gbrt_panelb_best_model_test_predictions_with_AD.csv")
...
test_ad_summary_df.to_csv(
    "gbrt_panelb_best_model_test_AD_summary.csv",
    index=False,
)

print("Saved best-model test AD summary to: gbrt_panelb_best_model_test_AD_summary.csv")


# ------------------------------------------------------------
# 13. Save final best model with AD reference
# ------------------------------------------------------------
...
torch.save(
    ...
    "best_gbdt_model_panelb_mean_imp_with_AD.pt",
)

print("Best overall GBDT model with AD saved as best_gbdt_model_panelb_mean_imp_with_AD.pt")


# ------------------------------------------------------------
# 14. Feature importance plots and saving
# ------------------------------------------------------------
    ...
    all_feature_importances.append(
        {
            ...
            "train_col_means": all_fold_imputation_means[fold_idx],
        }
    )
...
print(
    "Feature importance plots generated for all 25 models and saved to "
    "'all_feature_importances_panelb_mean_imp.pkl'."
)
