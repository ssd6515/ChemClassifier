# Model Training for Panel B: Logistic Regression with RDKit descriptors
import pandas as pd
import numpy as np
...
# ============================================================
# Logistic Regression ElasticNet Panel B
...
#
# Scaling:
#   StandardScaler is fit only on the imputed training set.
...
def build_applicability_domain_reference(
    ...
    Note:
    For LR, the model itself uses StandardScaler-transformed data.
    However, this AD function uses the imputed descriptor matrix and applies
    its own standardization internally. This keeps the AD definition consistent
    and keeps feature min/max values interpretable in the original descriptor scale.
...
print(
    "no_ecfp, LR-ElasticNet, no_SMOTE, "
    "C_pool = [0.1, 1.0, 10.0], "
    "l1l2_ratio = [0.25, 0.5, 0.75], "
    "rdkit, mean_imputation, correct RepeatedStratifiedKFold, "
...
# ------------------------------------------------------------
# 5. Create all RepeatedStratifiedKFold splits correctly
# ------------------------------------------------------------
...
for repeat in range(N_REPEATS):
    ...
    for k in range(N_SPLITS):
        ...
        train_feature_imputed = mean_impute_with_given_column_means(
            train_feature_raw,
            train_col_means,
        )
        valid_feature_imputed = mean_impute_with_given_column_means(
            valid_feature_raw,
            train_col_means,
        )
        test_feature_imputed = mean_impute_with_given_column_means(
            test_feature_raw,
            train_col_means,
        )

        print(
            "train_feature NAN count after imputation:",
            np.isnan(train_feature_imputed).sum(),
        )
        print(
            "valid_feature NAN count after imputation:",
            np.isnan(valid_feature_imputed).sum(),
        )
        print(
            "test_feature NAN count after imputation:",
            np.isnan(test_feature_imputed).sum(),
        )

        # ----------------------------------------------------
        # Scaling using training set only
        # ----------------------------------------------------

        scaler = StandardScaler()

        train_feature = scaler.fit_transform(train_feature_imputed)
        valid_feature = scaler.transform(valid_feature_imputed)
        test_feature = scaler.transform(test_feature_imputed)
        ...
        if best_valid_loss < global_best_val_loss:
            ...
            global_best_hyperparams = (best_c, best_r)
    ...
    repeat_best_scalers.append(repeat_best_scaler)
    ...
    print(
        ...
        f"C = {repeat_best_hyper[0]}, l1l2_Ratio = {repeat_best_hyper[1]}"
    )
...
# ------------------------------------------------------------
# 7. Save fold metrics and repeat-level metrics
# ------------------------------------------------------------

with open("results_lr_panelb_repeat_lr.pkl", "wb") as f:
    pickle.dump(
        {
...
# ------------------------------------------------------------
# 8. Compute final overall metrics across all 25 models
# ------------------------------------------------------------
...
with open("results_lr_panelb_final_metrics_lr.pkl", "wb") as f:
    pickle.dump(all_metrics, f)


# ------------------------------------------------------------
# 10. Select and save the overall best model
# ------------------------------------------------------------

best_overall_model = global_best_model
best_overall_scaler = global_best_scaler
...
print(
    ...
    f"\n  C = {best_overall_hyper[0]}"
    f"\n  l1l2_ratio = {best_overall_hyper[1]}"
)

# Save model, scaler, and imputation means together.
# This is important because new prediction data must go through:
#   1. mean imputation using train_col_means
#   2. StandardScaler transformation
#   3. LogisticRegression prediction
torch.save(
    {
    ...
    "best_lr_model_panelb.pt",
)

print("Best overall LR model, scaler, and imputation means saved as best_lr_model_panelb.pt")


# ------------------------------------------------------------
# 11. Applicability Domain analysis for selected best model
# ------------------------------------------------------------

print("\n============================================================")
print("Applicability Domain Analysis for Selected Best LR Model")
print("============================================================")
...
# Imputed features in original descriptor scale.
# These are used for AD and feature min/max reporting.
best_train_feature_imputed = mean_impute_with_given_column_means(
    best_train_feature_raw,
    best_overall_col_means,
)

best_valid_feature_imputed = mean_impute_with_given_column_means(
    best_valid_feature_raw,
    best_overall_col_means,
)

best_test_feature_imputed = mean_impute_with_given_column_means(
    best_test_feature_raw,
    best_overall_col_means,
)

# Scaled features for LR model prediction.
best_train_feature_scaled = best_overall_scaler.transform(best_train_feature_imputed)
best_valid_feature_scaled = best_overall_scaler.transform(best_valid_feature_imputed)
best_test_feature_scaled = best_overall_scaler.transform(best_test_feature_imputed)

# Build AD reference from the best model's imputed training set only.
ad_reference = build_applicability_domain_reference(
    X_train_imputed=best_train_feature_imputed,
    feature_names=feature_names,
    ad_k=AD_K,
    ad_distance_percentile=AD_DISTANCE_PERCENTILE,
)
...
ad_summary_df = pd.DataFrame(
    {
        ...
        "best_model_C": [best_overall_hyper[0]],
        "best_model_l1l2_ratio": [best_overall_hyper[1]],
        "n_ad_training_chemicals": [best_train_feature_imputed.shape[0]],
        "n_features": [best_train_feature_imputed.shape[1]],
    }
)

ad_summary_df.to_csv("lr_panelb_best_model_AD_summary.csv", index=False)

print("\nSaved AD summary to: lr_panelb_best_model_AD_summary.csv")

# Save min and max feature ranges of the best model's training set.
# These are calculated in the original imputed descriptor scale, not LR-scaled space.
feature_range_df = pd.DataFrame(
    {
        ...
        "lr_scaler_mean": best_overall_scaler.mean_,
        "lr_scaler_scale": best_overall_scaler.scale_,
    }
)

feature_range_df.to_csv("lr_panelb_best_model_training_feature_ranges.csv", index=False)

print("Saved training feature min/max ranges to: lr_panelb_best_model_training_feature_ranges.csv")
...
ad_training_distances_df.to_csv(
    "lr_panelb_best_model_training_AD_distances.csv",
    index=False,
)

print("Saved training AD distances to: lr_panelb_best_model_training_AD_distances.csv")


# ------------------------------------------------------------
# 12. Apply selected best LR model + AD to its held-out test set
# ------------------------------------------------------------

print("\n============================================================")
print("Best LR Model Test Set Predictions with AD Flags")
print("============================================================")

# Predict every test chemical.
# AD does not remove any chemical from prediction.
# LR prediction uses scaled features.
best_test_pred = best_overall_model.predict(best_test_feature_scaled)
best_test_proba = best_overall_model.predict_proba(best_test_feature_scaled)
...
# AD uses imputed descriptors in original descriptor scale.
best_test_ad_results = evaluate_applicability_domain(
    X_query_imputed=best_test_feature_imputed,
...
best_test_results_df.to_csv(
    "lr_panelb_best_model_test_predictions_with_AD.csv",
    index=False,
)

print("Saved best-model test predictions with AD to: lr_panelb_best_model_test_predictions_with_AD.csv")
...
test_ad_summary_df.to_csv(
    "lr_panelb_best_model_test_AD_summary.csv",
    index=False,
)

print("Saved best-model test AD summary to: lr_panelb_best_model_test_AD_summary.csv")


# ------------------------------------------------------------
# 13. Save final best LR model with AD reference
# ------------------------------------------------------------
...
torch.save(
    ...
    "best_lr_model_panelb_with_AD.pt",
)

print("Best overall LR model with AD saved as best_lr_model_panelb_with_AD.pt")
